...
def main3(argv):
    ...
    appuie2 = pygame.image.load("Images/appuie2.png").convert_alpha() 
    ...
    touche_actuelle.append(partition[0])
    index2 = 0
    ...
    while partition[index].time_pos == partition[index+1].time_pos:
   ...
   #     print("%d %d %d" % (color1.R + i*colorvars.R, color1.G + i*colorvars.G, color1.B + i*colorvars.B))
    ...
    while continuer:
            ...
            for items in inp.read(1000):
                ...
                if len(touche_actuelle) == 1 and touche_actuelle[0].posy+touche_actuelle[0].time>=531:
                    ...
                    if midi_value[1] == int(touche_actuelle[0].touche) and midi_value[0] == 144 and midi_value[2] != 0:
                        ...
                        while partition[index+plus].time_pos == partition[index+1+plus].time_pos:
                            touche_actuelle.append(partition[index+1+plus])
                            plus+=1
                    '''else:
        ...
        for i in range(int(taille2/2)):
            if c>=partition2[i].time_pos and partition2[i].posy+partition2[i].time-1 == fenetre.get_height()- clavier.get_height() and allume != partition2[i].time_pos:
                midiOut.note_on(partition2[i].touche,127)
                allume = partition2[i].time_pos
                
            if c>=partition2[i].time_pos and partition2[i].posy == fenetre.get_height()- clavier.get_height():
                midiOut.note_off(partition2[i].touche,127)
        ...
        for elem in grise:
            ...
            if simpl < 5:
                if simpl%2 == 0:
                    fenetre.blit(appuie,(30*int(simpl/2)+(int(elem/12)-2)*210, fenetre.get_height()- clavier.get_height()), (30*int(simpl/2),0,30,152))
                else:
                    fenetre.blit(appuie2,(43*int(simpl/2)+(int(elem/12)-2)*210, fenetre.get_height()- clavier.get_height()), (43*int(simpl/2),0,43,152))
            else:
                if simpl%2 == 1:
                    fenetre.blit(appuie,(30*int(((simpl%12)-5)/2)+93+(int(elem/12)-2)*210, fenetre.get_height()- clavier.get_height()), (30*int(((simpl%12)-5)/2)+93,0,30,152))
                else:
                    fenetre.blit(appuie2,(43*(int(simpl/2)-1)+(int(elem/12)-2)*210, fenetre.get_height()- clavier.get_height()), (43*(int(simpl/2)-1),0,43,152))
